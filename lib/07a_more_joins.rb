...
def top_track_counts
  ...
  execute(<<-SQL)
    ...
    GROUP BY
      albums.title
    ORDER BY
      COUNT(tracks.song) DESC LIMIT 10;
  SQL
end
...
def soundtrack_wars
  ...
  execute(<<-SQL)
    ...
    GROUP BY
      albums.artist
    ORDER BY
      COUNT(albums.asin) DESC LIMIT 1;
  SQL
end
...
def expensive_tastes
  ...
  execute(<<-SQL)
    ...
    SELECT
      styles.style, SUM(albums_with_tracks.price) / SUM(albums_with_tracks.tracks_per_album)
    FROM
      styles
    INNER JOIN
    (
      SELECT DISTINCT
        albums.asin, albums.price, COUNT(tracks.song) AS tracks_per_album
      FROM
        albums
      INNER JOIN
        tracks
      ON
        albums.asin = tracks.album
      GROUP BY
        albums.asin, albums.price
      HAVING
        COUNT(tracks.song) IS NOT NULL
      ) AS albums_with_tracks
    ON
      albums_with_tracks.asin = styles.album
    GROUP BY
      styles.style
    HAVING
      AVG(albums_with_tracks.price / albums_with_tracks.tracks_per_album) IS NOT NULL
    ORDER BY
      AVG(albums_with_tracks.price / albums_with_tracks.tracks_per_album) DESC LIMIT 5
  SQL
end
