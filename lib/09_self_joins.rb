...
def craiglockhart_and_tollcross
  ...
  execute(<<-SQL)
    ...
    INNER JOIN
      stops AS stopsa ON stopsa.id = a.stop_id
    INNER JOIN
      stops AS stopsb ON stopsb.id = b.stop_id
    WHERE
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    FROM
      routes as a1
    INNER JOIN
      routes as a2 ON a1.company = a2.company AND a1.num = a2.num
    INNER JOIN
      routes as b1 ON b1.stop_id = a2.stop_id
    INNER JOIN
      routes as b2 ON b1.company = b2.company AND b1.num = b2.num
    INNER JOIN
      stops as stopsb ON b2.stop_id = stopsb.id
    INNER JOIN
      stops as transfer_stops ON transfer_stops.id = b1.stop_id
    WHERE
      a1.stop_id = 53 AND stopsb.name = 'Sighthill'
  SQL
end
